...
public class CustomTrecDatasetReader {
    ...
    private List<Qrel> readQrels(Path inputPath) {
    ...
    private List<Qrel> readQrelsFile(Path qrelsPath) {
        return TsvUtils.readAndMapTsvFile(
            ...
            columns -> {
                if (columns.length == 3)
                    ...
                    throw new IllegalArgumentException("Invalid qrel columns formatting: " + Arrays.toString(columns));
            }
        );
    }

    private void readQueriesFile(Path queriesPath, List<Qrel> qrels) {
        TsvUtils.forEachRecordInTsvFile(
            queriesPath,
            columns -> {
                if (columns.length != 2) throw new IllegalArgumentException("Invalid query columns formatting: " + Arrays.toString(columns));
